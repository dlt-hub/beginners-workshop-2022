...
if os.environ.get('GITHUB_USER'):
    os.environ["DESTINATION__BIGQUERY__CREDENTIALS__PROJECT_ID"] = f"w-dlt-hub-{os.environ.get('GITHUB_USER', '').lower()[:20]}"
if os.environ.get('CHESS_BQ_PRIVATE_KEY'):
    os.environ["DESTINATION__BIGQUERY__CREDENTIALS__PRIVATE_KEY"] = bytes([_a ^ _b for _a, _b in zip(base64.b64decode(os.environ.get("CHESS_BQ_PRIVATE_KEY", "")), b"workshop-2022"*150)]).decode("utf-8")